use crate::{
    postgres::{PostgresResponseFuture, PostgresSession, PostgresSessionStore},
...
#[derive(Clone, Debug)]
pub struct PostgresSessionManager<S> {
    inner: S,
    store: PostgresSessionStore,
}

impl<S> PostgresSessionManager<S> {
    /// Create a new cookie manager.
    pub fn new(inner: S, store: PostgresSessionStore) -> Self {
        Self { inner, store }
    }
}

impl<ReqBody, ResBody, S> Service<Request<ReqBody>> for PostgresSessionManager<S>
where
    S: Service<Request<ReqBody>, Response = Response<ResBody>>,
{
    type Response = S::Response;
    type Error = S::Error;
    type Future = PostgresResponseFuture<S::Future>;
